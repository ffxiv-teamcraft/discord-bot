...
export class PowershellCommand implements Command {
    ...
    async run(parsedUserCommand: CommandContext): Promise<void> {
		...
		.addFields([
			...
			value: "1. Press the Windows key, search ``env`` and hit enter \n 2. Click ``Environment Variables`` in the advanced tab \n 3. Under \"**System Variables**\", locate the Path entry \n 4. Click ``New`` and paste ``%SystemRoot%\\system32\\WindowsPowerShell\\v1.0`` \n 5. Click OK to confirm all this work, then try reopening TC!"
			},
			{
			name: "Update from Windows 7",
			value: "This issue basically only happens on Windows 7 or lower, and we actually no longer properly support anything below Windows 10. Most programs and apps no longer do. There exist free ways to update as far as Win10."
			}
		])
        .setFooter({
