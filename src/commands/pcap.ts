...
export class PcapCommand implements Command {
    ...
    async run(parsedUserCommand: CommandContext): Promise<void> {
		...
		.addField("Teamcraft still doesn't think I have Npcap installed even though I just reinstalled it!", "Type the ``!!powershell`` command here in the server and follow the instructions.")
		.addField("What if you *did* get output from ``window.debugPackets = true;``, but your inventory still isn't working right?", "Type the ``!!ichara`` command here in the server and follow the instructions, as it's possible you need to reset your character links.")
